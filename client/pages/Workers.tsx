...
import { getYearData, saveYearData, formatDateString } from "@/utils/yearStorage";
...
export default function Workers() {
  ...
  useEffect(() => {
    const saved = getYearData<Employee[]>("employees", selectedYear, []);
    setWorkers(saved);
  }, [selectedYear]);

  // Auto-save workers whenever they change
  useAutoSave({
    data: workers,
    key: "employees",
    year: selectedYear,
  ...
  const saveWorkers = (updated: Employee[]) => {
    setWorkers(updated);
  ...
  const handleAddWorker = () => {
    ...
    const newWorker: Employee = {
      ...
      paymentStatus: formData.paymentStatus || "active",
    ...
    saveWorkers(updated);
    setFormData({ paymentStatus: "active", paymentMethod: "direct_deposit" });
    setIsAddModalOpen(false);
    toast({ title: "Worker Added", description: `${newWorker.name} added successfully` });
  };
  ...
  const handleEditWorker = () => {
    ...
    const updated = workers.map((w) =>
      w.id === editingWorkerId
        ? {
            ...w,
            name: formData.name || w.name,
            position: formData.position || w.position,
            weeklyRate: formData.weeklyRate || w.weeklyRate,
            startDate: formData.startDate || w.startDate,
            paymentMethod: formData.paymentMethod || w.paymentMethod,
            paymentStatus: formData.paymentStatus || w.paymentStatus,
            email: formData.email || w.email,
            telephone: formData.telephone || w.telephone,
            address: formData.address || w.address,
            tin: formData.tin || w.tin,
          }
        : w
    );
  ...
  const handleDeleteWorker = (id: string) => {
    ...
    toast({
      ...
      description: `${worker?.name} removed successfully`,
    });
  };
  ...
  const printWorkersList = () => {
    try {
      ...
      // Summary
      const totalWeeklyPayroll = workers.reduce((sum, w) => sum + w.weeklyRate, 0);
      const activeWorkers = workers.filter((w) => w.paymentStatus === "active").length;
      pdf.setFontSize(9);
      ...
      workers.forEach((worker) => {
        ...
        // Weekly Rate column
        pdf.text(`$${worker.weeklyRate.toLocaleString()}`, xPosition + cellPadding, yPosition);
        xPosition += colWidths[3];

        // Start Date column
        const startDateFormatted = new Date(worker.startDate).toLocaleDateString();
        pdf.text(startDateFormatted, xPosition + cellPadding, yPosition);
        xPosition += colWidths[4];

        // Payment Method column
        let paymentMethod = worker.paymentMethod
          ? worker.paymentMethod.charAt(0).toUpperCase() + worker.paymentMethod.slice(1).replace(/_/g, " ")
          : "-";
        ...
        // Status column
        const status = worker.paymentStatus ? worker.paymentStatus.charAt(0).toUpperCase() + worker.paymentStatus.slice(1) : "Active";
    ...
    try {
      ...
      workers.forEach((worker, index) => {
        ...
        const detailLines = [
          ...
          { label: "Weekly Rate:", value: `$${worker.weeklyRate.toLocaleString()}` },
          { label: "Start Date:", value: formatDateString(worker.startDate) },
          { label: "Payment Method:", value: worker.paymentMethod ? worker.paymentMethod.charAt(0).toUpperCase() + worker.paymentMethod.slice(1).replace(/_/g, " ") : "-" },
          { label: "Status:", value: worker.paymentStatus ? worker.paymentStatus.charAt(0).toUpperCase() + worker.paymentStatus.slice(1) : "Active" },
        ];
        ...
        const personalLines = [
          ...
          { label: "Social/TIN:", value: worker.tin || "-" },
        ];
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={formData.paymentMethod || "direct_deposit"}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="tin">Social/TIN</Label>
                    <Input
                      id="tin"
                      value={formData.tin || ""}
                ...
                <tbody>
                  {workers.map((worker, idx) => (
                    <tr key={worker.id} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                      ...
                      <td className="p-3 text-slate-700 whitespace-nowrap">${worker.weeklyRate.toLocaleString()}</td>
                      <td className="p-3 text-slate-700 whitespace-nowrap">{formatDateString(worker.startDate)}</td>
                      <td className="p-3">
                        <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${getStatusColor(worker.paymentStatus)}`}>
                          {worker.paymentStatus || "active"}
                        </span>
                      </td>
                      <td className="p-3">
                        <div className="flex gap-2">
        ...
        <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-tin">Social/TIN</Label>
                <Input
                  id="edit-tin"
                  value={formData.tin || ""}
                  onChange={(e) => setFormData({ ...formData, tin: e.target.value })}
